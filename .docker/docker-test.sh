...
function test_request_nginx(){
    url=$1
    result=false
    echo "Sending request to nginx via '$url' and expect to get 200 OK response"
    curl -s -I ${url} | grep -q "200 OK" && result=true
    assert_result ${result}
}
...
function test_composer_installed(){
    service=$1
    result=false
    echo "Testing composer installed on '$service'"
    docker_exec ${service} composer --version | grep -q "Composer" && result=true
    assert_result ${result}
}
...
test_composer_installed ${service}
